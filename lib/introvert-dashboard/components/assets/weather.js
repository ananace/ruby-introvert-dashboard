var weather = {
  apply: function(input) {
    ...
    for (i in days) {
      ...
      weather.applyTitle(day, curdata, data);

      var id = "weather" + day.charAt(0).toUpperCase() + day.substring(1);
      //$('#' + id + ' .card-title').html(weather.generateTitle(day, curdata, data));
      $('#' + id + ' .card-subtitle').html(weather.generateSubtitle(curdata));
    }
  },

  applyTitle: function(day, curdata, data) {
    let card = $('#weather.card');

    $('#weather i[data-value="weather.' + day +'.icon"]')
      .attr('class', 'wi wi-' + curdata.symbol);
  ...
  generateTitle: function(day, curdata, data) {
  ...
  generateSubtitle: function(curdata) {
    const subtitle = '<i class="wi wi-wind from-'+Math.round(curdata.wind.dir)+'-deg mr-1"></i>'+curdata.wind.speed+'<small>m/s</small>&nbsp;\n'+
